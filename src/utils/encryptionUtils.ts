...
/**
 * Generate client ID in format PD-XXX
 * Supports existing IDs with prefix JASI- or PD- to continue numbering sequentially.
 */
export function generateClientId(existingIds: string[] = []): string {
  let maxNum = 0;
  
  existingIds.forEach(id => {
    const match = id.match(/^(?:JASI|PD)-(\d+)$/);
    if (match) {
  ...
  return `PD-${String(newNum).padStart(3, '0')}`;
}

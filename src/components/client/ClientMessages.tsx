  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto h-full flex flex-col">
      ...
      <div className="space-y-4">
        <div>
          <h2 className="text-3xl font-bold text-midnight mb-2">Messagerie</h2>
          <p className="text-gray-600">Communiquez directement avec notre équipe MIDEESSI</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-green-700 uppercase tracking-wide">Messages reçus</p>
                <p className="text-2xl font-bold text-green-900">{messages.filter(m => m.expediteur === 'admin').length}</p>
              </div>
            ...
            Object.entries(groupedMessages).map(([date, dayMessages]) => (
              <div key={date} className="space-y-4">
                ...
                <div className="flex items-center gap-3 my-4">
                  ...
                  <span className="text-xs font-semibold text-gray-500 uppercase px-3 bg-white rounded-full">
                ...
                {dayMessages.map(message => (
                  ...
                  >
                    {/* Avatar */}
                    <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold text-[var(--brand-midnight)] ${
                      ...
                      <div
                        className={`px-5 py-3 rounded-2xl shadow-sm transition-all ${
                          message.expediteur === 'client'
                            ...
                            : 'bg-gray-100 text-midnight rounded-bl-sm border border-gray-200'
                        }`}
                      >
        ...
        <div className="border-t border-gray-200 p-6 bg-gradient-to-t from-gray-50 to-white">
          <form onSubmit={handleSendMessage} className="space-y-3">
            <textarea
              ...
              className="w-full px-4 py-3 rounded-xl bg-white border-2 border-gray-200 text-midnight placeholder-gray-500 focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed resize-none sm:px-5 sm:py-4"
            />
            <div className="flex items-end justify-between">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-gold to-yellow-400 hover:from-gold/90 hover:to-yellow-400/90 text-midnight text-sm font-semibold transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed sm:px-6 sm:py-3"
              >
                {sending ? (
                  <>

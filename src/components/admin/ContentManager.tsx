...
import { createDynamicProject, createDynamicTeamMember, createRecruitmentOffer, getDynamicProjects, getDynamicTeamMembers, getRecruitmentOffers } from '../../lib/contentManagement';
...
const ContentManager = () => {
  ...
  useEffect(() => {
    const handler = () => {
      setRecruitmentOffers(getRecruitmentOffers());
  ...
  const submitRecruitment = (e: React.FormEvent) => {
    e.preventDefault();
      ...
      {activeTab === 'projects' && (
        <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          <form onSubmit={submitProject} className="space-y-4 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <input value={projectForm.image} onChange={(e) => setProjectForm({ ...projectForm, image: e.target.value })} placeholder="URL image" className="w-full rounded-2xl border border-slate-200 px-4 py-3" />
      ...
      {activeTab === 'team' && (
        <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          <form onSubmit={submitTeam} className="space-y-4 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <input value={teamForm.image} onChange={(e) => setTeamForm({ ...teamForm, image: e.target.value })} placeholder="URL image" className="w-full rounded-2xl border border-slate-200 px-4 py-3" />
